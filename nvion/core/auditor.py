...
class NAuditor:
    ...
    Custom rule packs:
        auditor = NAuditor(
            custom_patterns=[
                (r"internal_api_key", BlockReason.CREDENTIAL_EXFILTRATION),
                (r"prod_db_password", BlockReason.FORBIDDEN_CONTENT),
            ],
            custom_checks=[my_check_function],
        )
    """
    ...
    def __init__(
        ...
        """
        Parameters:
            custom_patterns  — List of (regex_pattern, BlockReason) tuples.
                               Added to the base forbidden patterns.
            custom_checks    — List of callables: fn(output_str, agent) -> Optional[str]
                               Return a string reason to block, None to pass.
            max_output_chars — Global max output size override (default 100,000).
        """
        ...
        # Compile base + custom patterns together
        self._all_patterns = FORBIDDEN_PATTERNS + [
            (p, r) for p, r in self._custom_patterns
        ]
    ...
    def audit(
        ...
        # ── Check 2b: Custom rule pack patterns ───────────────────────────
        if self._custom_patterns:
            checks.append("custom_pattern_check")
            lower = output_str.lower()
            for pattern, block_reason in self._custom_patterns:
                if re.search(pattern, lower):
                    return self._block(
                        agent_id, action, checks, block_reason,
                        f"Custom rule pack blocked output: pattern '{pattern}' matched.",
                    )
        ...
        # ── Check 5b: Custom check functions ──────────────────────────────
        if self._custom_checks:
            checks.append("custom_checks")
            for check_fn in self._custom_checks:
                try:
                    result = check_fn(output_str, agent)
                    if result:
                        return self._block(
                            agent_id, action, checks,
                            BlockReason.FORBIDDEN_CONTENT,
                            f"Custom check blocked output: {result}",
                        )
                except Exception:
                    pass  # Custom checks never crash the auditor
    ...
    def _check_size_cap(self, output_str: str, agent) -> Optional[str]:
        """Enforce maximum output size (global + agent-specific)."""
            ...
            for cap in agent.risk_caps:
                if "max output" in cap.lower():
                    try:
                        ...
                        max_chars = min(max_chars, num)
                    except (AttributeError, ValueError):
                        pass

        if len(output_str) > max_chars:
            return (
                f"Output size {len(output_str):,} chars exceeds "
