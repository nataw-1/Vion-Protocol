...
class IdentityRegistry:
    ...
    def _save_suspended_state(self, system_halted: bool = False, condition_counts: dict = None):
        """
        Persist suspended agent IDs, system HALT state, and condition counts
        to governance.state. Survives process crashes.
        """
        suspended = [
    ...
    def load_halt_state(self) -> dict:
        """Load persisted HALT state and condition counts on boot."""
        if not self._state_path.exists():
            return {"system_halted": False, "condition_counts": {}}
        try:
            data = json.loads(self._state_path.read_text(encoding="utf-8"))
            return {
                "system_halted": data.get("system_halted", False),
    ...
    def validate_scope(self, agent_id: str, requested_action: str) -> dict:
        ...
        # ── Extract explicit DENIED resource names from permissions ──────────
        # Supports both formats:
        #   "Financial transactions          : DENIED"
        #   "DENIED: send money, wire transfer"
        explicit_denied_terms = []
        for permission in agent.permissions:
            p_upper = permission.upper()
            if "DENIED" in p_upper:
                # Extract the action description before ": DENIED"
                clean = re.sub(r":\s*DENIED\s*$", "", permission, flags=re.IGNORECASE).strip()
                clean = re.sub(r"^DENIED\s*:\s*", "", clean, flags=re.IGNORECASE).strip()
                # Split by comma for multi-term denied entries
                terms = [t.strip().lower() for t in clean.split(",") if len(t.strip()) > 2]
                explicit_denied_terms.extend(terms)
        ...
        # ── Check explicit DENIED terms extracted from permissions ───────────
        for denied_term in explicit_denied_terms:
            denied_words = [w for w in denied_term.split() if len(w) > 3]
            if denied_words and any(dw in action_lower for dw in denied_words):
                return {
                    "valid": False,
                    "reason": (
                        f"Action '{requested_action}' matches explicitly DENIED "
                        f"permission term '{denied_term}' for agent {agent_id}. "
