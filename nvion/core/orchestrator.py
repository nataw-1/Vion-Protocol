...
class Orchestrator:
    ...
    def __init__(self):
        ...
        # Load persisted HALT state from governance.state (survives crashes)
        boot_state = self.registry.load_halt_state()

        self.halt_engine  = HaltEngine(
            self.logger, self.telegram,
            persist_fn=self.registry._save_suspended_state,
